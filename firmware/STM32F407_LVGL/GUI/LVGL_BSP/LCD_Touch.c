...
{
	uint32_t i;
	for(i=0; i<us*8; i++);  // 粗略延时，需根据实际时钟调整
}
...
//x,y:坐标
//color:颜色
void TP_Drow_Touch_Point(uint16_t x,uint16_t y,uint32_t color)
{
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void LCD_DrawRoughLine(uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2,uint32_t color)
{
	uint16_t t; 
...
uint8_t TP_Init(void)
{
	...
	if(Adujust)
	{
		...
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=0.086822;
			tp_dev.xoff=-20;
			tp_dev.yfac=0.129794;
			tp_dev.yoff=-15;
		}else if(USE_HORIZONTAL==1)
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=-0.085340;
			tp_dev.xoff=339;
			tp_dev.yfac=-0.128355;
			tp_dev.yoff=494;
