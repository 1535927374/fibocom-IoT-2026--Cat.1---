...
![登录页面](./img/99e2-1.png.jpg)

登录好后点击「开发者中心」。

![开发者中心](./img/99e2-2.png.jpg)

## 4.1 创建产品
接下来我们先创建一个产品，之后再创建具体的设备。

![创建产品按钮](./img/99e2-3.png.jpg)

可按照下图参数创建产品。

![产品参数配置](./img/99e2-4.png.jpg)

## 4.2 创建物模型
通过物模型我们可以定义设备的属性、服务和事件功能。我们需要创建几个物模型，用于上传数据和事件告警。

![点击产品开发](./img/99e2-5.png.jpg)
...
![物模型参数设置](./img/99e2-6.png.jpg)

## 4.3 创建设备
接下来就开始创建产品下的具体设备。

![添加设备弹窗](./img/99e2-7.png.jpg)

## 4.4 生成MQTT三元组
**MQTT 三元组**是 MQTT 协议中至关重要的，就像去考试的时候，一定要带上准考证、身份证才能进考场，要有「MQTT 三元组」才能连接 MQTT 服务端。

![设备列表点击详情](./img/99e2-8.png.jpg)

![获取设备ID、产品ID、设备密钥](./img/99e2-9.png.jpg)
...
![下载token工具](./img/99e2-10.png.jpg)

下载好 token 生成工具，打开界面如下。

![token工具界面](./img/99e2-11.png.jpg)
...
![时间戳转换网页](./img/99e2-12.png.jpg)

根据介绍，填好各个参数的空，我们选择 sha1 的加密方式，大家可以选择自己喜欢的。填好如下操作：

![生成token操作](./img/99e2-13.png.jpg)
...
![服务器地址文档](./img/99e2-14.png.jpg)

### 4.5.2 订阅主题
选择设置直连设备属性：
`{device-name}` 是设备ID，比如我们的就是 temp01。

![订阅主题](./img/99e2-15.png.jpg)

### 4.5.3 上报主题
选择直连设备上报属性：
`{device-name}` 是设备ID，比如我们的就是 temp01。

![上报主题](./img/99e2-16.png.jpg)
